...
// 活動列表 + 公告 API，單一資料來源 POST /site/activity
const ActiveInit = async () => {
  try {
    common.loading = true
    const res = await common.baseAjax("POST", "/site/activity")
    if (res.code == 1) {
      state.bulletContents = res.data.activity ?? []
...
const claimPromotion = async (val) => {
  try {
    common.loading = true
...
// showInfoSpread 改由 ActiveInit 取得 API 後依 bulletContents 初始化
    ...
    <div class="category-list">
      ...
      >
        <div class="category-bottom">
          <div class="first-line" v-if="item.category?.length">
            <div class="flag-box">
              <div
                class="flag"
          ...
          >
            ...
            <div class="rContent">
              ...
              >
                ...
                <div class="limit" v-if="item.start_time || item.end_time">
                  <div v-if="item.start_time">開始時間: {{ item.start_time }}</div>
                  <div v-if="item.end_time">結束時間: {{ item.end_time }}</div>
                </div>
                <div class="bonus">
                </div>
                <div class="acContents">
                  <div class="content" v-if="content.content1" v-html="content.content1"></div>
                  <div class="content" v-if="content.content2" v-html="content.content2"></div>
                  <div class="content" v-if="content.content3" v-html="content.content3"></div>
                </div>
              </div>
              <div class="gift">
                <div 
                @click="claimPromotion(String(item.id))"
                class="base-button">領取獎勵</div>
              </div>
            </div>
            </div>
          </div>
