...
const gameType = ref(route.query.gameType)
...
const state = reactive({
  ...
  gameGroupData: {},
  idCodeMap: {},
  makeCodeMap: () => {    // 製作 gameTypeId 和 code 的匹配表
    for (let i = 0; i < common.menu.length; i++) {
      state.idCodeMap[common.menu[i].code] = common.menu[i].id
    }
  },
  gameInit: async () => {
    try {
      if (common.menu.length == 0) {        // 確認有無 id
        await common.menuInit();
      }
      common.loading = true;
      state.makeCodeMap();
      const fetchGameData = async () => {
        let form = { id: state.idCodeMap[gameType.value] }; // 動態 ID
      ...
      await fetchGameData(); // 確保在有 id 的情況下才執行
    } catch (error) {
      console.log(error);
    }
  },
...
if (Object.keys(state.gameGroupData).length == 0) {   // 避免 reload() 之後沒資料
  state.gameInit();
}
...
//const previousRoute = window.previousRoute.fullPath
  ...
  <div class="gamelist-wrapper">
    ...
    <div class="games">
      <div class="group-banner">
        <img :src="state.gameGroupData.banner" />
      </div>
      <div
